...
import { createTurnCorrelator, effectiveEnds, sharedTraceIds } from "@/lib/effect/turn-correlation";
...
describe("per-turn traces (one trace per turn - e.g. the DPT producer)", () => {
  ...
  const assigned = (tools: AgentToolCall[] = [], events: Event[] = []) => createTurnCorrelator(turns).assign(tools, events);
...
describe("turn_id producers", () => {
  ...
  it("does not time-fallback an explicit but unresolved turn_id", () => {
    const calls = [call({ turn_id: "missing-turn", started_at: "2026-08-12T08:00:05.000Z" })];
    expect(assigned(calls).toolsFor(turns[0]!.id)).toHaveLength(0);
    expect(assigned(calls).toolsFor(turns[1]!.id)).toHaveLength(0);
  });
});
