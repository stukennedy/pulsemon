...
function enqueueMessage(
  ...
  if (!deps.queue) {
    return Effect.fail(new MissingConfigError({ message: "Telemetry queue is not configured" }));
  }

  const total = operationCount(batch);
  if (total === 0 && sampledOut === 0) {
    return Effect.fail(new ValidationError({ message: "No valid records in batch" }));
  }
  if (total > 1000) {
    return Effect.fail(new PayloadTooLargeError({ message: "Max 1000 operations per batch" }));
  }
  ...
  if (total === 0) return Effect.succeed(accepted(message));
...
export function writeTelemetryQueueMessage(
  env: Pick<Env, "DB" | "INGEST_CARDINALITY_MAX_VALUES_PER_KEY">,
